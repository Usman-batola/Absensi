...
<div class="max-w-2xl mx-auto">
    ...
    <form method="POST" class="bg-white p-8 rounded-lg shadow-lg">
        ...
        <div class="mb-6">
            ...
            <input type="email" name="email" value="<?= $user['email'] ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="lokasi_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" required>
                ...
                <?php foreach ($lokasi as $l): ?>
                    <option value="<?= $l['id'] ?>" <?= ($userLokasi && $userLokasi['lokasi_id'] == $l['id']) ? 'selected' : '' ?>>
                        <?= $l['name'] ?> (Radius: <?= $l['radius'] ?>m)
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
